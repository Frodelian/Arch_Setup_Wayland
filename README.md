...
Предположим, вы хотите отредактировать файл, принадлежащий пользователю root, и у вас есть привилегии sudo.

Это можно сделать следующим образом:
```
sudo vim /etc/fstab.
```
Сделав это, вы откроете vim, используя конфигурацию пользователя root, поэтому вы не сможете использовать плагины и настройки, которые вы сделали в своем редакторе.

Но есть альтернативный способ сделать это.

Сначала нужно открыть терминал и задать переменную окружения с путём к редактору, я это задаю в файле .zshenv. Затем можно открыть файл, используя sudoedit:
...
sudoedit /etc/fstab
```
Теперь вы сможете редактировать этот файл как пользователь root, но сохраните все настройки, которые вы сделали в neovim. Он откроется как ваш пользователь, но сохранится как пользователь root.

Но в некоторых случаях, sudoeditможет произойти сбой с ошибкой ниже:
```
sudoedit /etc/fstab
sudoedit: /etc/fstab: editing files in a writable directory is not permitted
```
Чтобы это исправить, необходимо выполнить следующую команду:
```
sudo visudo
```
Затем в начале файла, в разделе «По умолчанию», добавьте еще одну строку со следующим содержимым:
```
Defaults        !sudoedit_checkdir
```
Сохраните файл и запустите его. sudoeditС этого момента всё будет работать, и это сообщение об ошибке больше не будет вас беспокоить.
